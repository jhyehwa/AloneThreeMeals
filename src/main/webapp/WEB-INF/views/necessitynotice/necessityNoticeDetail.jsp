...
<link rel="stylesheet" href="resources/style/NecessityNoticeDetail.css"
	type="text/css">
<script>
$(function(){
	$('#update').click(function(){
		$('#necessityUpdate').submit();
	})
	
	$('#back').click(function(){
		location.replace("necessityNoticeList");
	});
	...
	$('#delete').click(function(){
		...
		var n_Seq = $('#n_Seq').val();
		if(message == true){
			location.replace("necessityNoticeDelete?n_Seq="+n_Seq);
		}
	});
	...
	$('#commentIn').click(function(){
		...
		if(!c_Content){
         ...
         $.ajax({
            type : "post",
            url : "necessityCommentRegProcess",
            data : param,
...
<div class="wrapper">		
		<div id="header">
			...
			<form id="necessityUpdate" action="necessityNoticeUpdateView" method="get">
			<div id="top">
				<input type="hidden" id="n_Seq" name="n_Seq"
				value="${necessity.n_Seq}" />
				<div id="content">
					<ul>
						<li class="title"><span>제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목&nbsp;&nbsp;&nbsp;</span>${necessity.n_Title}</li>
						<li class="writer"><span>작&nbsp;성&nbsp;자&nbsp;&nbsp;&nbsp;</span>${necessity.n_Writer}</li>
						<li class="day"><span>작성일자&nbsp;&nbsp;</span>${dateview}</li>
						<li class="hit"><span>조&nbsp;회&nbsp;수&nbsp;&nbsp;&nbsp;</span>${necessity.n_Hit}</li>
						<li class="grok"><span>공&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;감&nbsp;&nbsp;&nbsp;</span>${grok}</li>
						<li class="nongrok"><span>비&nbsp;공&nbsp;감&nbsp;&nbsp;&nbsp;</span>${nongrok}</li>
						<li class="content">${necessity.n_Content}</li>
					</ul>
				</div>
			</div>
			<div>
				<c:if test="${m_Id eq necessity.n_Writer}">
		...
		<form action="" method="get">
				...
				<c:if test="${necessityCommentList.size() > 0}">
					<c:forEach var="i" begin="0" end="${necessityCommentList.size()-1}">
						<ul>
						<c:if test="${necessityCommentList[i].c_Grok eq 'yes'}">
							<li class="grok yes">공감</li>
						</c:if>
						<c:if test="${necessityCommentList[i].c_Grok eq 'no'}">
							<li class="grok no">비공감</li>
						</c:if>
							<li><span class="writer">ID : </span>${necessityCommentList[i].c_Writer}</li>
							<li class="c_Content"><span class="content">내용 : </span>${necessityCommentList[i].c_Content}</li>
						...
						<c:if test="${necessityCommentList[i].c_Writer eq m_Id}">
						<li>
							<input type="button" class="commentUpdate" value="수정"/>
							<input type="hidden" class="c_Seq" value="${necessityCommentList[i].c_Seq}"/>
				...
				<div id="paging">
					...
					<a class="page" href="necessityNoticeDetail?n_Seq=${necessity.n_Seq}&currPage=${i}">${i}</a>
					</c:forEach>
				</div>
				<div class="comment">
