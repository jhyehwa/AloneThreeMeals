...
<link rel="stylesheet" href="resources/style/AnabadaNoticeDetail.css"
	type="text/css">
<script>
$(function(){
	$('#update').click(function(){
		$('#anabadaUpdate').submit();
	});
	
	$('#back').click(function(){
		location.replace("anabadaNoticeList");
	});
	...
	$('#delete').click(function(){
		...
		var n_Seq = $('#n_Seq').val();
		if(message == true){
			location.replace("anabadaNoticeDelete?n_Seq="+n_Seq);
		}
	});
...
<body>
	<div class="wrapper">
		...
		<form id="anabadaUpdate" action="anabadaNoticeUpdateView" method="get">
			<div id="top">
				<input type="hidden" id="n_Seq" name="n_Seq"
					value="${anabada.n_Seq}" />
				<div id="content">
					<ul>
						<li class="title"><span>제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목&nbsp;&nbsp;&nbsp;</span>${anabada.n_Title}</li>
						<li class="writer"><span>작&nbsp;성&nbsp;자&nbsp;&nbsp;&nbsp;</span>${anabada.n_Writer}</li>
						<li class="day"><span>작성일자&nbsp;&nbsp;</span>${dateview}</li>
						<li class="hit"><span>조&nbsp;회&nbsp;수&nbsp;&nbsp;&nbsp;</span>${anabada.n_Hit}</li>
						<li class="grok"><span>공&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;감&nbsp;&nbsp;&nbsp;</span>${grok}</li>
						<li class="nongrok"><span>비&nbsp;공&nbsp;감&nbsp;&nbsp;&nbsp;</span>${nongrok}</li>
						<li class="content">${anabada.n_Content}</li>
					</ul>
				</div>
			</div>
			<div>
				<c:if test="${m_Id eq anabada.n_Writer}">
		...
		<form action="" method="get">
			<div class="commentList">
				<c:if test="${anabadaCommentList.size() > 0}">
					<c:forEach var="i" begin="0" end="${anabadaCommentList.size()-1}">
						<ul>
							<c:if test="${anabadaCommentList[i].c_Grok eq 'yes'}">
								<li class="grok yes">공감</li>
							</c:if>
							<c:if test="${anabadaCommentList[i].c_Grok eq 'no'}">
								<li class="grok no">비공감</li>
							</c:if>

							<li><span class="writer">ID : </span>${anabadaCommentList[i].c_Writer}</li>
							<li class="c_Content"><span class="content">내용 : </span>${anabadaCommentList[i].c_Content}</li>
							...
							<c:if test="${anabadaCommentList[i].c_Writer eq m_Id}">
								<li>
									<input type="button" class="commentUpdate" value="수정" />
									<input type="hidden" class="c_Seq" value="${anabadaCommentList[i].c_Seq}" />
			...
			<div id="paging">
				...
				<a class="page" href="anabadaNoticeDetail?n_Seq=${anabada.n_Seq}&currPage=${i}">${i}</a>
				</c:forEach>
			</div>
			<div class="comment">
				...
				<div class="commnetWrite">
					<ul>
						<li>댓글</li>
						<li>
							<p id="c_seq"></p> <textarea rows="4" cols="66" id="contents"></textarea>
						</li>
						<li class="radio_btn">
							<input type="radio" name="grok" id="yes" value="yes" checked="checked">공감
							<input type="radio" name="grok" id="no" value="no">비공감</li>
						<li>
							<input type="button" id="update_btn" value="수정" />
							<input type="button" id="cancle_btn" value="취소" />
							<input type="button" id="commentIn" value="입력"/></li>
					</ul>
				</div>  
			</div>
		</form>
