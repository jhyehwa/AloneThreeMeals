...
<link rel="stylesheet" href="resources/style/MyRecipeNoticeList.css"
   type="text/css">
<script>
$(function(){
	$('#login_result').hide();
	...
	$('.page').click(
			function() {
				...
				$(this).attr(
						'href',
						'myRecipeNoticeList?find='+find+'&qtext='+qtext+ '&currPage='+currPage);
			});
...
<body>
   <div id="wrapper">
      ...
      <form action="myRecipeNoticeList" method="get">
      <div class="place">
      ...
      <div class="notice">
        <c:if test="${myRecipelist.size() > 0}">
            <c:forEach var="i" begin="0" end="${myRecipelist.size()-1}">
               <ul>
                  <%-- <li class="n_Seq" id="n_seq${i}">${storelist[i].n_Seq}</li> --%>
                  <%-- <li><input type="hidden" id="n_seq${i}" value="${storelist[i].n_Seq}"/></li> --%>
                  <li class="n_Picture"><a href="myRecipeNoticeDetail?n_Seq=${myRecipelist[i].n_Seq}"><img class="recipeImg" src="upload/${myRecipelist[i].n_Picture}" width="230px" height="230px"/></a></li>
                  ...
                  <li class="n_Hit">${myRecipelist[i].n_Hit}</li>
               </ul>
            </c:forEach>
         </c:if>
     </div>
     <form id="regProcess" action="myRecipeNoticeRegView" method="post">
     <div id="reg">
