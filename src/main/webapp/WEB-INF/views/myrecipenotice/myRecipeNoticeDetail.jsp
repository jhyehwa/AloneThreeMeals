...
<link rel="stylesheet" href="resources/style/MyRecipeNoticeDetail.css"
	type="text/css">
<script>
	$(function(){
		$('#update').click(function(){
			$('#myRecipeUpdate').submit();
		})
		
		$('#back').click(function(){
			location.replace("myRecipeNoticeList");
		});
		...
		$('#delete').click(function(){
			...
			var n_Seq = $('#n_Seq').val();
			if(message == true){
				location.replace("myRecipeNoticeDelete?n_Seq="+n_Seq);				
			}
		});
		...
		$('#commentIn').click(function(){
			...
			var param = "n_Seq="+n_Seq +"&c_Writer="+c_Writer+"&c_Content="+c_Content +"&c_Grok="+ c_Grok;
...
<body>
	<div class="wrapper">
			...
			<form id="myRecipeUpdate" action="myRecipeNoticeUpdateView" method="get">
			<div id="top">
				<input type="hidden" id="n_Seq" name="n_Seq" value="${myRecipe.n_Seq}" />
				<ul>
					<li class="title"><span>제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${myRecipe.n_Title}</li>
					<li class="writer"><span>작&nbsp;성&nbsp;자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${myRecipe.n_Writer}</li>
					<li class="day"><span>작성일자&nbsp;&nbsp;&nbsp;&nbsp;</span>${dateview}</li>
					<li class="hit"><span>조&nbsp;회&nbsp;수&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${myRecipe.n_Hit}</li>
					<li class="grok"><span>공&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;감&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${grok}</li>
					<li class="nongrok"><span>비&nbsp;공&nbsp;감&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${nongrok}</li>
					<li class="star"><span>별&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;점&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>${myRecipe.n_Star}</li>
					<li class="img"><img src="upload/${myRecipe.n_Picture}" width="150px" height="150px"></li>
					<li class="content">${myRecipe.n_Content}</li>
				</ul>
				<div>
				<c:if test="${m_Id eq myRecipe.n_Writer}">
		...
		<form action="" method="get">
				...
				<c:if test="${myRecipeCommentList.size() > 0}">
					<c:forEach var="i" begin="0" end="${myRecipeCommentList.size()-1}">
						<ul>
						<c:if test="${myRecipeCommentList[i].c_Grok eq 'yes'}">
							<li class="grok yes">공감</li>
						</c:if>
						<c:if test="${myRecipeCommentList[i].c_Grok eq 'no'}">
							<li class="grok no">비공감</li>
						</c:if>
						
						<li><span class="writer">ID : </span>${myRecipeCommentList[i].c_Writer}</li>
						<li class="c_Content"><span class="content">내용 : </span>${myRecipeCommentList[i].c_Content}</li>
						...
						<c:if test="${myRecipeCommentList[i].c_Writer eq m_Id}">
						<li>
							<input type="button" class="commentUpdate" value="수정"/>
							<input type="hidden" class="c_Seq" value="${myRecipeCommentList[i].c_Seq}"/>
				...
				<div id="paging">
					...
					<a class="page" href="myRecipeNoticeDetail?n_Seq=${myRecipe.n_Seq}&currPage=${i}">${i}</a>
					</c:forEach>
				</div>
				<div class="comment">
				...
				<div class="commnetWrite">
					<ul>
						<li>댓글</li>
						<li><p id="c_seq"></p>
						<textarea rows="4" cols="66" id="contents"></textarea></li>
						<li>
